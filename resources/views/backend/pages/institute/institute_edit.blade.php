...
@section('backend_content')
    <div class="add-student-div">
        <form id="InsregForm" action="{{ route('institute.information.update') }}" method="post">
        <input type="hidden" name="id" value="{{$institute->id}}" 
                            @error('name') is-invalid @enderror value="{{ $institute->name }}">
            @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 fs-5">
                        ...
                        <input type="text" placeholder="type..." name="type" id="type"
                            @error('type') is-invalid @enderror value="{{ $institute->type }}">
                        @error('type')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-sm-6 fs-5">
                        ...
                        <input type="text" placeholder="established..." name="established" id="established"
                            @error('established') is-invalid @enderror value="{{ $institute->established }}">
                        @error('established')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-sm-6 fs-5">
                        ...
                        <input type="text" name="ranking" id="ranking"
                            @error('ranking') is-invalid @enderror value="{{ $institute->ranking }}">
                        @error('ranking')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <label for="about" class=" col-form-label text-start">About Institute</label>
                                <div class="">
                                    <textarea class="form-control summernote @error('about') is-invalid @enderror" name="about"
                                        id="about">{{$institute->about}} </textarea>
                                    @error('about')
                                        <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                </div>
